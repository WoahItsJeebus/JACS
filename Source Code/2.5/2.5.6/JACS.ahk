...
CreateGui(*) {
	...
	AddTipBox() {
		...
		tipBox := MainUI.Add("Text", "x+0 w" UI_Margin_Width " h" tipHeight " BackgroundTrans vTipBox", "") ; wide on purpose
		tipBox.SetFont("s10 w550 Italic", "Consolas")

		; Store scrolling data
		...
		; Start initial tip + scroll
		LoadNewTip()
		; TipTimer := SetTimer(() => LoadNewTip(), Random(30000, 60000))
	...
	LoadNewTip() {
		...
		for index, tip in tips {
			if !ArrayHasValue(lastIndexes, index)
