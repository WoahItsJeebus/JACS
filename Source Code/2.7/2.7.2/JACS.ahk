...
ClickWindow(optionalHWND := "") {
	...
	loop requestedClicks {
		...
		if timedMode {
			...
			; Always move before checking the target. The old implementation checked
			; hoverWindow first and then made its click decision from that stale value.
			MouseMove(clickX, clickY, MouseSpeed ? Random(0, MouseSpeed) : 0)
			Sleep(25)
		}

		; Roblox may expose its renderer as a child control. Compare the root
		; window instead of requiring MouseGetPos's control output to be empty.
		if !IsMouseOverTarget(targetHwnd)
			break

		Click(mouseButton)
		clickCount++

		if A_Index < requestedClicks
			Sleep(Random(25, Max(25, MouseClickRateOffset ? MouseClickRateOffset : 25)))
	}

	return clickCount > 0

	GetClickPoint(targetHwnd, &clickX, &clickY) {
		local clientX := 0, clientY := 0, clientWidth := 0, clientHeight := 0
		try WinGetClientPos(&clientX, &clientY, &clientWidth, &clientHeight, "ahk_id " targetHwnd)
		catch
			return false
		...
		; Keep randomization inside the client area even when a saved radius is
		; larger than the current Roblox window.
		...
		clickX := Round(clientX + (clientWidth / 2) + Random(-radiusX, radiusX))
		clickY := Round(clientY + (clientHeight / 2) + Random(-radiusY, radiusY))
		return true
	}

	IsMouseOverTarget(targetHwnd) {
		local hoverWindow := 0
		try MouseGetPos(,, &hoverWindow)
		catch
			return false
		...
		local rootWindow := DllCall("GetAncestor", "ptr", hoverWindow, "uint", 2, "ptr")
		return (rootWindow ? rootWindow : hoverWindow) == targetHwnd
	}
